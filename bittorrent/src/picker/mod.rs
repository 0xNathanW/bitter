...
impl Picker {
    ...
    pub async fn pick_blocks(
        ...
        while remaining != 0 {
            if let Some(idx) = self.piece_picker.write().await.pick_new_piece() {
...
mod tests {
    ...
    async fn test_pick_blocks() {
        ...
        let requests_1 = picker.pick_blocks(&HashSet::new(), 4).await;
        assert_eq!(requests_1.len(), 4);
        let requests_2 = picker.pick_blocks(&HashSet::new(), 4).await;
    ...
    async fn test_pick_blocks_end_game() {
        ...
        let requests_3 = picker.pick_blocks(&previous_requests, 4).await;
