...
impl Gui {
    pub fn new(_cc: &eframe::CreationContext<'_>) -> Self {
        Self { widgets: Widgets::new(), torrent_loaded: false, app: None }
    }
}
        ...
        if !ctx.input().raw.dropped_files.is_empty() {
            ...
            if let Some(path) = &file.path {
                if path.extension() == Some(std::ffi::OsStr::new("torrent")) {
                    ...
                    let torrent = core::torrent::Torrent::new(path).unwrap();
                    self.widgets.items.push(Box::new(crate::widgets::info::TorrentInfo(Rc::new(torrent))));
                }
            }
        }
    }    
